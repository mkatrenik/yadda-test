...
widgets.login = {
  loginToApp: function(browser) {
    return browser
        .setValue(selectors.userField, 'foo')
        .setValue(selectors.passwordField, 'letMe1n')
...
module.exports = function() {
    ...
    this.given(/I open the (url|site) "$string"$/, function (type, page, done) {
        ...
        .given(/^I have signed in$/, function(done) {
            const { login: { loginToApp } } = widgets;
            loginToApp(this.browser).call(done);
        })
        ;
